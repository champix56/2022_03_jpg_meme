...
exports.initMemeCreatorView = exports.loadSelectWithImages = exports.setMemeValuesInFormInputs = exports.initForm = exports.setMemeOnSVGViewer = void 0;
/**
...
exports.setMemeOnSVGViewer = setMemeOnSVGViewer;
/**
...
exports.setMemeValuesInFormInputs = setMemeValuesInFormInputs;
/**
...
exports.loadSelectWithImages = loadSelectWithImages;
function initMemeCreatorView(wrapperNode, id) {
    //appel xhr mais avec promise ES6
    var promiseCreator = fetch(location.origin + '/views/meme-creator.html')
        //1ere etape de reception -> transformation du flux recu en text
        .then(function (f) {
        return f.text();
    });
    //deuxieme ressources
    var promiseViewer = fetch(location.origin + '/views/meme-svg-viewer.html').then(function (f) { return f.text(); });
    //reception rest par promise avec template string pour ladresse
    var promiseImages = fetch("".concat(ARD_REST_SRV, "/images"))
        .then(function (f) {
        return f.json();
    });
    Promise.all([promiseCreator, promiseViewer, promiseImages])
        ...
        //partie 3 les datas du rest (images)
        listeGlobalImages = resp[2];
        //selection du meme
        if (undefined !== id) {
            fetch("".concat(ARD_REST_SRV, "/memes/") + id).then(function (f) { return f.json(); })
                .then(function (r) {
                unMemeGlobal = r;
...
exports.initMemeCreatorView = initMemeCreatorView;
